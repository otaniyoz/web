...
{#if $page.data && tableData}
  <section>
    <Heading level={1} classes="grid-col17">
      ...
      <div id='project-categories' slot='subtitles'>
        {#each $page.data.categories as category}
          <Link classes='{(category===$page.params.category)?'grey-color':''} regular-weight regular-size large-line-height regular-right-margin' href='{base}/projects/{category}' outside={false} target='_self'>{typographString(category)}</Link>
        {/each}
      </div>
    </Heading>
    <Table {tableData} />
