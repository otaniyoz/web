<script lang="ts">
  ...
  sortedNotes = sortedNotes.filter(note => note.categories.includes('Books'));
  ...
  <Heading level={1}>
    ...
    <svelte:fragment slot='subtitles'>
      {#each allCategories as category}
        <Link classes='{(category==='Books')?'grey-color':''} regular-weight regular-size large-line-height regular-right-margin' href='{base}/notes/{category.toLowerCase()}' outside={false} target='_self'>{category}</Link>
      {/each}
    </svelte:fragment>
  </Heading>
  <Table {tableData} />
