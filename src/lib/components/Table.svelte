      ...
      {#each tableData.tableRows as row, rowIndex}
        <tr class:grey-color={!tableData.tableLinks[numColumns * rowIndex]}>
          {#each row as column, columnIndex}
            <td class:title={tableData.tableAccents[numColumns * rowIndex + columnIndex]}>
              <p class:text-right={tableData.tableColumns[columnIndex] === "date"}>
                {#if tableData.tableLinks && tableData.tableLinks[numColumns * rowIndex + columnIndex]}
                  <a href={tableData.tableLinks[numColumns * rowIndex + columnIndex]}>{typographString(column)}</a>
                {:else}
